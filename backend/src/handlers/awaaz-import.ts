...
import { batchCreateCases } from '../repository/awaaz-case.repository';
import { ok, badRequest, serverError } from '../utils/response';
import type { AwaazImportResult } from '../models/awaaz-case';
...
async function awaazImportHandler(event: AuthenticatedEvent): Promise<APIGatewayProxyResultV2> {
  try {
    if (!event.body) {
    ...
    console.log(`[AwaazImport] User ${event.user.userId} importing ${rows.length} cases`);
    ...
    const result = await batchCreateCases(casesData);

    const response: AwaazImportResult = {
    ...
    console.error('Error importing awaaz cases:', err);
    return serverError(err);
  }
}

export const handler = withAuth(awaazImportHandler, { requiredModule: 'cfm', requiredRole: 'admin' });
